...
class DataMgMutationAdminController extends Controller
{
    public function index()
    {
        return view('admin.migas-mutation', [
    ...
    public function store(Request $request){
        ...
        if ($validator->fails()){
            return redirect('/admin/mutasi/migas')->with('error', 'Input data gagal, mohon periksa kembali.')
        ...
        if (\DB::table('migas_stocks')->where('name', $request->name)->count() > 0){
            if ($request->item_in == 0){
                ...
                if ((int)$qty-(int)$request->item_out){
                    return redirect('/admin/mutasi/migas')->with('error', 'Input data gagal, jumlah barang keluar tidak sesuai dengan stock.')
                    ->withErrors($validator)
                    ->withInput();
                }
    ...
    public function destroy(MigasMutation $mutation){
        ...
        return redirect('/admin/mutasi/migas')->with('success', 'Data berhasil dihapus');
    }
}
