...
class DataMgStockController extends Controller
{
    ...
    {
        return view('superadmin.migas-stock', [
            'data' => MigasStock::all()
        ]);
    }
    ...
    public function store(Request $request){
        ...
        if ($validator->fails()){
            return redirect('/superadmin/stock/migas')->with('error', 'Input data gagal mohon periksa kembali.')
            ->withErrors($validator)
            ->withInput();
        }

        if (MigasStock::where('name', $request->name)->count() > 0) {
            return redirect('/superadmin/stock/migas')->with('error', 'Input gagal, data barang sudah ada. Silahkan update jumlah.');
         }

        MigasStock::create($request->all());
        return redirect('/superadmin/stock/migas')->with('success', 'Data berhasil ditambahkan');
    }
    ...
    public function update(Request $request, MigasStock $stock){
        ...
        return redirect('/superadmin/stock/migas')->with('success', 'Data berhasil di update');
    }
