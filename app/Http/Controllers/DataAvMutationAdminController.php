...
class DataAvMutationAdminController extends Controller
{
    public function index()
    {
        return view('admin.aviasi-mutation', [
    ...
    public function store(Request $request){
        ...
        if ($validator->fails()){
            return redirect('/admin/mutasi/aviasi')->with('error', 'Input data gagal, mohon periksa kembali.')
        ...
        if (\DB::table('aviasi_stocks')->where('name', $request->name)->count() > 0){
            if ($request->item_in == 0){
                ...
                if ((int)$qty-(int)$request->item_out){
                    return redirect('/admin/mutasi/aviasi')->with('error', 'Input data gagal, jumlah barang keluar tidak sesuai dengan stock.')
                    ->withErrors($validator)
                    ->withInput();
                }
        ...
        return redirect('/admin/mutasi/aviasi')->with('success', 'Data berhasil ditambahkan');
    }
    ...
    public function update(Request $request, AviasiMutation $mutation){
        ...
        return redirect('/admin/mutasi/aviasi')->with('success', 'Data berhasil diupdate');
    }
    ...
    public function destroy(AviasiMutation $mutation, Request $request){
        ...
        return redirect('/admin/mutasi/aviasi')->with('success', 'Data berhasil dihapus');
    }
}
